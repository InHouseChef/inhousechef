...
export const MenuItem: MenuItemComponent = ({ children, onClick, active, ...rest }) => {
    ...
    return (
        <button
            ...
            onClick={handleClick}
            className={clsx(
                'disabled:text-gray700 flex items-center gap-1 whitespace-nowrap rounded border-none bg-transparent p-1.5 text-sm leading-5 transition-colors',
                'hover:bg-primary hover:text-white disabled:pointer-events-none',
                '[&:disabled>svg]:text-gray700 [&>svg]:text-gray [&:hover>svg]:text-white [&>svg]:h-6 [&>svg]:w-6 [&>svg]:transition-colors',
                '[&:hover>a]:text-white',
                {
                    'text-primary': active,
