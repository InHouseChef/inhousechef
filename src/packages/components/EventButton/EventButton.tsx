...
export const EventButton = forwardRef<HTMLButtonElement, EventButtonProps>(
    ({ children, type = 'button', isLoading, className, ...rest }, ref) => (
        <button
            className={`flex h-max items-center gap-1 rounded border-0 bg-transparent p-0 text-black transition-colors duration-200 hover:text-primary disabled:text-grey800 ${className}`}
            type={type}
            ref={ref}
            {...rest}>
